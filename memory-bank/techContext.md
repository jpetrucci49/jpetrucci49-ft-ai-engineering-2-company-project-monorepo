...
13. **M6.5 telemetry capture** — Backoffice `lib/telemetry/` queues events and POSTs `{ events: [...] }` to `NEXT_PUBLIC_TELEMETRY_ENDPOINT`. FastAPI stub `POST /telemetry/events` validates the envelope and returns `{ received: N }` (no persistence). `TELEMETRY_ENDPOINT` is read on the API for the env pattern. All UI capture goes through `track()`.
